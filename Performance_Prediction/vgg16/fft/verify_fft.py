#!/usr/bin/env python3
"""
Verification Script: Compare PyTorch Default vs ai3 FFT

This script proves that ai3 FFT is actually being used by running
both PyTorch (default cuDNN) and ai3 (explicit FFT) side-by-side
...
def main():
    print("="*80)
    print("VERIFICATION: PyTorch Default vs ai3 FFT")
    print("="*80)
    ...
    print("\n" + "="*80)
    print("TEST 2: ai3 with Explicit FFT Algorithm")
    print("="*80)

    # Load model and convert to ai3 FFT
    ai3_model = models.vgg16(weights=models.VGG16_Weights.DEFAULT)
    ai3_model.eval()

    print("\nApplying ai3.swap_conv2d with 'fft' algorithm...")
    ai3.swap_conv2d(ai3_model, 'fft')

    # Inspect layers
    ai3_ai3, ai3_pytorch = inspect_model_layers(ai3_model, "ai3 Model")

    # Measure
    print("\nMeasuring ai3 FFT performance...")
    ...
    print(f"   ai3 FFT:         {ai3_stats['mean']:.2f}ms")
    ...
    if abs(diff_pct) > 5:  # More than 5% difference
        ...
        if diff_pct > 0:
            print(
                f"   ✓ VERIFIED: ai3 FFT is SLOWER (expected - explicit algorithm choice)")
        else:
            print(f"   ✓ VERIFIED: ai3 FFT is FASTER (explicit FFT beats auto-select)")
    ...
    if ai3_ai3 == 13 and ai3_pytorch == 0:
        print("✓ All 13 Conv2D layers were replaced with ai3 FFT implementation")
        print("✓ Layer inspection confirms ai3 algorithm='fft' is active")
        print(
            f"✓ Performance difference ({diff_pct:+.1f}%) confirms different execution")
        print("\n→ CONFIRMED: Your profiling is using ai3 FFT, NOT PyTorch default!")
    else:

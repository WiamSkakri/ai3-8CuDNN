"""
Model-level (overall) ENERGY prediction.

Trains 4 ML models (XGBoost, LightGBM, Random Forest, Neural Network)
to predict log1p(energy_per_inference_j) for whole-model inference.

Uses architecture-level features plus device hardware specs.
Requires combined_overall.csv which already has energy columns.
"""
...
def leave_one_model_out(
    ...
    print(f"\n{'=' * 60}")
    print("  LEAVE-ONE-MODEL-OUT CROSS-VALIDATION (MODEL ENERGY)")
    print(f"{'=' * 60}")
...
def main():
    print("=" * 60)
    print("  MODEL-LEVEL ENERGY PREDICTION")
    print("=" * 60)

    X, _y_time, y_energy = load_overall_data(DATA_PATH)

    if y_energy is None:
        print("\nERROR: 'energy_per_inference_j' column not found "
              f"in {DATA_PATH}.")
        print("This script requires overall CSVs with power/energy columns.")
        sys.exit(1)
    ...
    print(f"Target: energy_per_inference_j")

    X_train, X_test, y_train, y_test = train_test_split(
        X, y_energy, test_size=0.2, random_state=SEED)
    ...
    for name, builder in MODEL_BUILDERS.items():
        ...
        artifact_path = os.path.join(
            ARTIFACT_DIR, f'model_energy_{name}.joblib')
    ...
    print(f"\n\n{'=' * 60}")
    print("  TEST SET COMPARISON (MODEL ENERGY)")
    print(f"{'=' * 60}")
    ...
    if model_col is not None:
        ...
        leave_one_model_out(
            X_with_model, y_energy, numerical_cols, categorical_cols)

#!/usr/bin/env python3
"""
ResNet152 Implicit GEMM Performance Profiling with Power Monitoring - FULL VERSION

This script adds power consumption monitoring to the performance profiling,
running on 100 input sizes from 224x224 to 512x512 using Implicit GEMM algorithm.
"""

from resnet_152_gemm import PowerMonitor, CUDALayerTimer, measure_overall_performance, format_tuple_value, print_cuda_info
import torch
...
def main():
    ...
    print("=" * 80)
    print("RESNET152 IMPLICIT GEMM PERFORMANCE + POWER PROFILING - FULL VERSION")
    print("=" * 80)
    ...
    # Use implicit_gemm in filename (replace space with underscore)
    algo_filename = ALGORITHM.replace(' ', '_')
    overall_csv = os.path.join(
        results_dir, f"{MODEL_NAME}_{algo_filename}_{device_name}_overall.csv")
    layers_csv = os.path.join(
        results_dir, f"{MODEL_NAME}_{algo_filename}_{device_name}_layers.csv")

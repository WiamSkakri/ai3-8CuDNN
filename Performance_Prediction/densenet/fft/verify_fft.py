#!/usr/bin/env python3
"""
Verification Script: Compare PyTorch Default vs ai3 FFT (DenseNet121)
...
def main():
    print("="*80)
    print("VERIFICATION: PyTorch Default vs ai3 FFT (DenseNet121)")
    print("="*80)
    ...
    if not ai3.using_cudnn():
        print("✗ ai3 cuDNN not available - cannot test FFT algorithm")
        return False
    ...
    # Test 2: ai3 with FFT
    print("\n" + "="*80)
    print("TEST 2: ai3 DenseNet121 (Explicit FFT)")
    print("="*80)
    print("Converting to ai3 with 'fft' algorithm...")
    ai3_model = ai3.swap_conv2d(densenet, 'fft')
    ...
    print(
        f"ai3 (fft):          {ai3_stats['mean']:.2f}ms ± {ai3_stats['std']:.2f}ms")
    ...
    print("="*80)
    print(
        f"✓ ai3 FFT algorithm successfully applied ({ai3_count} layers converted)")

#!/bin/bash
#SBATCH --job-name=vgg16_implicit_gemm_h100
#SBATCH --output=vgg16_implicit_gemm_results_h100/vgg16_implicit_gemm_h100_%j.out
#SBATCH --error=vgg16_implicit_gemm_results_h100/vgg16_implicit_gemm_h100_%j.err
...
ls -la vgg16_implicit_gemm.py 2>/dev/null || echo "  ⚠ WARNING: vgg16_implicit_gemm.py not found in current directory!"
echo ""

# Verify script exists
if [ ! -f "vgg16_implicit_gemm.py" ]; then
    echo "✗ ERROR: vgg16_implicit_gemm.py not found!"
    echo "Expected location: $(pwd)/vgg16_implicit_gemm.py"
    echo "Please ensure you submit the job from the directory containing vgg16_implicit_gemm.py"
    exit 1
fi

# Create results directory
RESULTS_DIR="vgg16_implicit_gemm_results_h100"

#!/bin/bash
#SBATCH --job-name=resnet152_gemm_l40s
#SBATCH --output=resnet152_gemm_results_l40s/resnet152_gemm_l40s_%j.out
#SBATCH --error=resnet152_gemm_results_l40s/resnet152_gemm_l40s_%j.err
...
ls -la resnet_152_gemm.py 2>/dev/null || echo "  ⚠ WARNING: resnet_152_gemm.py not found in current directory!"
echo ""

# Verify script exists
if [ ! -f "resnet_152_gemm.py" ]; then
    echo "✗ ERROR: resnet_152_gemm.py not found!"
    echo "Expected location: $(pwd)/resnet_152_gemm.py"
    echo "Please ensure you submit the job from the directory containing resnet_152_gemm.py"
    exit 1
fi

# Create results directory
RESULTS_DIR="resnet152_gemm_results_l40s"

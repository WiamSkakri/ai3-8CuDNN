#!/bin/bash
#SBATCH --job-name=densenet_gemm_l40s
#SBATCH --output=densenet_gemm_results_l40s/densenet_gemm_l40s_%j.out
#SBATCH --error=densenet_gemm_results_l40s/densenet_gemm_l40s_%j.err
...
ls -la densenet_gemm.py 2>/dev/null || echo "  ⚠ WARNING: densenet_gemm.py not found in current directory!"
echo ""

# Verify script exists
if [ ! -f "densenet_gemm.py" ]; then
    echo "✗ ERROR: densenet_gemm.py not found!"
    echo "Expected location: $(pwd)/densenet_gemm.py"
    echo "Please ensure you submit the job from the directory containing densenet_gemm.py"
    exit 1
fi

# Create results directory
RESULTS_DIR="densenet_gemm_results_l40s"

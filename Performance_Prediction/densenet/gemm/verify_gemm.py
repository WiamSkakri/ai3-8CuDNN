#!/usr/bin/env python3
"""
Verification Script: Compare PyTorch Default vs ai3 GEMM

This script proves that ai3 GEMM is actually being used by running
both PyTorch (default cuDNN) and ai3 (explicit GEMM) side-by-side
...
def main():
    print("="*80)
    print("VERIFICATION: PyTorch Default vs ai3 GEMM - DenseNet121")
    print("="*80)
    ...
    pytorch_model = models.densenet(weights=models.DenseNet121_Weights.DEFAULT)
    pytorch_model.eval()
    ...
    print("\n" + "="*80)
    print("TEST 2: ai3 with Explicit GEMM Algorithm")
    print("="*80)

    # Load model and convert to ai3 GEMM
    ai3_model = models.densenet(weights=models.DenseNet121_Weights.DEFAULT)
    ai3_model.eval()

    print("\nApplying ai3.swap_conv2d with 'gemm' algorithm...")
    ai3.swap_conv2d(ai3_model, 'gemm')

    # Inspect layers
    ai3_ai3, ai3_pytorch = inspect_model_layers(ai3_model, "ai3 Model")

    # Measure
    print("\nMeasuring ai3 GEMM performance...")
    ai3_stats = measure_time(ai3_model, input_data, "ai3 GEMM", use_cuda)
    ...
    print(f"   ai3 GEMM:        {ai3_stats['mean']:.2f}ms")
    ...
    if abs(diff_pct) > 5:  # More than 5% difference
        ...
        if diff_pct > 0:
            print(
                f"   ✓ VERIFIED: ai3 GEMM is SLOWER (expected - explicit algorithm choice)")
        else:
            print(f"   ✓ VERIFIED: ai3 GEMM is FASTER (explicit GEMM beats auto-select)")
    ...
    if ai3_ai3 > 0 and ai3_pytorch == 0:
        print(f"✓ All {ai3_ai3} Conv2D layers were replaced with ai3 GEMM implementation")
        print("✓ Layer inspection confirms ai3 algorithm='gemm' is active")
        print(

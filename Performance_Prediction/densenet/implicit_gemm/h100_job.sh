#!/bin/bash
#SBATCH --job-name=densenet_implicit_gemm_h100
#SBATCH --output=densenet_implicit_gemm_results_h100/densenet_implicit_gemm_h100_%j.out
#SBATCH --error=densenet_implicit_gemm_results_h100/densenet_implicit_gemm_h100_%j.err
...
ls -la densenet_implicit_gemm.py 2>/dev/null || echo "  ⚠ WARNING: densenet_implicit_gemm.py not found in current directory!"
echo ""

# Verify script exists
if [ ! -f "densenet_implicit_gemm.py" ]; then
    echo "✗ ERROR: densenet_implicit_gemm.py not found!"
    echo "Expected location: $(pwd)/densenet_implicit_gemm.py"
    echo "Please ensure you submit the job from the directory containing densenet_implicit_gemm.py"
    exit 1
fi

# Create results directory
RESULTS_DIR="densenet_implicit_gemm_results_h100"
